...
void server::onDisconnected(QTcpSocket* client,int OptimalIndexThread)
{
    ...
    --СongestionThreadClients[OptimalIndexThread];
    if( !client) {qWarning() <<"error cast";}
...
server::server(QObject *parent) : QTcpServer(parent), Value_idealThreadCount(QThread::idealThreadCount()) {

    AvailableThreads.reserve(Value_idealThreadCount);
    СongestionThreadClients.reserve(Value_idealThreadCount);
...
server::~server()
{
    for (QThread *singleThread : AvailableThreads) {
        singleThread->quit(); // https://doc.qt.io/qt-6/qthread.html#quit
        singleThread->wait(); // https://doc.qt.io/qt-6/qthread.html#wait
    }
}
...
void server::incomingConnection(qintptr socketDescripter)
{
    ...
    //_________Thread__________________________________________________________________________________________________


        int OptimalIndexThread = AvailableThreads.size(); // имеет место быть ввиду reserve в ChatServer и if...else ниже

        if (OptimalIndexThread != Value_idealThreadCount) { //we can add a new thread

            AvailableThreads.append(new QThread(this));
            СongestionThreadClients.append(1);
            AvailableThreads.last()->start();

        } else {

            // ввиду заполнения слева направо минимум оптимальнее искать с конца, или напутал?
            const auto ItMinСongestionThreadClients = std::min_element(СongestionThreadClients.cbegin(), СongestionThreadClients.cend());
            OptimalIndexThread = std::distance(СongestionThreadClients.cbegin(),ItMinСongestionThreadClients);

            ++СongestionThreadClients[OptimalIndexThread];
        }

        client->moveToThread(AvailableThreads.at(OptimalIndexThread));


        // вместо bind лучше использовать лямбду!

        connect(AvailableThreads.at(OptimalIndexThread), &QThread::finished, client, &QObject::deleteLater);

         connect(client, &QTcpSocket::disconnected, this, [=]() { this->onDisconnected(client, OptimalIndexThread);});
